...
namespace GSPAWN
{
    public class ModularWallsObjectSpawn : ObjectSpawnTool
    {
        ...
        public void onModularWallPrefabProfileExamplePrefabChanged()
        {
            selectSpawnGuidePrefab();
        }
        ...
        {
            if (!wallPrefabProfile.isAnyPrefabUsed(ModularWallRuleId.WallSegment))
            {
                Debug.LogError("No WallSegment prefabs were found in profile '" + wallPrefabProfile.profileName + "'. Make sure " +
                    "the WallSegment rule has at least one prefab assigned to it that is marked as 'Used'.");
                return false;
            }
        ...
        private void connectLastToPenultimate()
        {
            ...
            if (cornerType == CornerType.Outer)
            {
                _lastSegment.setStartPosition(connectionCell.objectOBBCenter +
                    _penultimateSegment.extensionAxis * wallPrefabProfile.segment_ForwardPushDistance_Outer +
                    _lastSegment.extensionAxis * wallPrefabProfile.segment_InnerPushDistance_Outer);
            }
            else
            {
                _lastSegment.setStartPosition(connectionCell.objectOBBCenter +
                    _penultimateSegment.extensionAxis * wallPrefabProfile.segment_ForwardPushDistance_Inner +
                    _lastSegment.extensionAxis * wallPrefabProfile.segment_InnerPushDistance_Inner);
            }
        }
        ...
        private void spawnObjects()
        {
            ...
            for (int pieceIndex = 0; pieceIndex < numWallPieces; ++pieceIndex)
            {
                ...
                if (wallPiece.isImaginaryCornerPart) ruleId = ModularWallRuleId.WallSegment;
                switch (ruleId)
                {
                    case ModularWallRuleId.WallSegment:

                        objectPosition = refCell.objectOBBCenter + refCell.objectOBBRotation * wallPrefabProfile.segmentRT.position;
                        objectRotation = refCell.objectOBBRotation * wallPrefabProfile.segmentRT.rotation;
                        break;
                ...
                {
                    // Note: When no inner/outer corners are used, we are simulating them with a segment piece.
                    var pickRuleId = wallPiece.ruleId;
                    if (wallPiece.isImaginaryCornerPart) pickRuleId = ModularWallRuleId.WallSegment;
                    ...
                    if (wallPiece.ruleId != ModularWallRuleId.WallSegment && !wallPiece.isImaginaryCornerPart)
        ...
        private void processWallPieces()
        {
            ...
            for (int i = 0; i < numPieces; ++i)
            {
                ...
                else
                {
                    if (nextPiece != null)
                    {
                        ...
                        {
                            piece.ruleId    = ModularWallRuleId.WallSegment;
                            piece.refCell   = piece.stack.getCell(0);
                        }
                    }
                    else
            ...
            if (numPieces > 1)
            {
                ...
                {
                    if (Mathf.Abs(d0 - wallPrefabProfile.segment_ForwardPushDistance_Inner) < 1e-4f &&
                        Mathf.Abs(d1 - wallPrefabProfile.segment_ForwardPushDistance_Inner) < 1e-4f)
                    {
                ...
                {
                    if (Mathf.Abs(d0 - wallPrefabProfile.segment_ForwardPushDistance_Outer) < 1e-4f &&
                        Mathf.Abs(d1 - wallPrefabProfile.segment_ForwardPushDistance_Outer) < 1e-4f)
                    {
                        if (!firstPiece.isFirstCornerPart && !lastPiece.isSecondCornerPart) 
                        {
                            firstPiece.isSecondCornerPart   = true;
        ...
        private void spawnPillars()
        {
            ...
            for (int i = 0; i < numWallPieces; ++i)
            {
                ...
                if (wallPiece.ruleId == ModularWallRuleId.WallSegment && wallPrefabProfile.hasInnerCornerPillarBegin)
                {
                    // Spawn 2 pillars for both caps
                    spawnPillar(wallTransform.position, wallPiece, wallPrefabProfile.pillarMidSegmentBeginRT, ModularWallPillarRole.OuterCornerEnd);
                    spawnPillar(wallTransform.position, wallPiece, wallPrefabProfile.pillarMidSegmentEndRT, ModularWallPillarRole.InnerCornerBegin);
                }
                else
                if (wallPiece.ruleId == ModularWallRuleId.InnerCorner && wallPiece.isFirstCornerPart)
                {
                    if (wallPrefabProfile.hasInnerCornerPillarBegin)
                    {
                        ...
                        if (!wallPrefabProfile.hasLCornerPrefabs) parentPosition = wallPrefabProfile.calcWallSegmentOBB(wallPiece.gameObject).center;

                        GameObject pillar = spawnPillar(parentPosition, wallPiece, wallPrefabProfile.pillarInnerCornerBeginRT, ModularWallPillarRole.InnerCornerBegin);
                        if (!wallPrefabProfile.hasLCornerPrefabs && wallPiece.nextPiece != null)
                    ...
                    if (!wallPrefabProfile.hasLCornerPrefabs && wallPrefabProfile.hasInnerCornerPillarMiddle)
                    {
                ...
                if (wallPiece.ruleId == ModularWallRuleId.InnerCorner && wallPiece.isSecondCornerPart)
                {
                    if (wallPrefabProfile.hasInnerCornerPillarEnd)
                    {
                        ...
                        GameObject pillar = spawnPillar(parentPosition, wallPiece, wallPrefabProfile.pillarInnerCornerEndRT, ModularWallPillarRole.InnerCornerEnd);
                        if (!wallPrefabProfile.hasLCornerPrefabs && wallPiece.previousPiece != null)
                    ...
                    if (wallPrefabProfile.hasOuterCornerPillarMiddle && !wallPrefabProfile.hasLCornerPrefabs)
                    {
                ...
                if (wallPiece.ruleId == ModularWallRuleId.OuterCorner && wallPiece.isFirstCornerPart)
                {
                    if (wallPrefabProfile.hasOuterCornerPillarBegin)
                    {
                        ...
                        GameObject pillar = spawnPillar(parentPosition, wallPiece, wallPrefabProfile.pillarOuterCornerBeginRT, ModularWallPillarRole.OuterCornerBegin);
                ...
                if (wallPiece.ruleId == ModularWallRuleId.OuterCorner && wallPiece.isSecondCornerPart)
                {
                    if (wallPrefabProfile.hasOuterCornerPillarEnd)
                    {
                        ...
                        GameObject pillar = spawnPillar(parentPosition, wallPiece, wallPrefabProfile.pillarOuterCornerEndRT, ModularWallPillarRole.OuterCornerEnd);
                        if (!wallPrefabProfile.hasLCornerPrefabs && wallPiece.previousPiece != null)
                    ...
                    if (wallPrefabProfile.hasInnerCornerPillarMiddle && !wallPrefabProfile.hasLCornerPrefabs)
                    {
            ...
            // If the last and first piece form an inner corner...
            ...
            if (wallPrefabProfile.hasLCornerPrefabs && wallPrefabProfile.hasInnerCornerPillarMiddle)
            {
                if (lastPiece.closesWallLoop && lastPiece.closedCornerType == CornerType.Inner)
                {
                    ...
                    spawnPillar(parentPosition, lastPiece, wallPrefabProfile.pillarInnerCornerMiddleRT, ModularWallPillarRole.InnerCornerMiddle);
                }
            }

            // If the last and first piece form an outer corner...
            if (!wallPrefabProfile.hasLCornerPrefabs && wallPrefabProfile.hasOuterCornerPillarMiddle)
            {
                float absDot = Vector3Ex.absDot(firstPiece.segment.extensionAxis, lastPiece.segment.extensionAxis);
                if (absDot < 1e-5f)
                {
                ...
                if (wallPrefabProfile.hasLCornerPrefabs && wallPrefabProfile.hasOuterCornerPillarMiddle)
                {
                    ...
                    {
                        Vector3 parentPosition = lastPiece.gameObject.transform.position;
                        spawnPillar(parentPosition, lastPiece, wallPrefabProfile.pillarOuterCornerMiddleRT, ModularWallPillarRole.OuterCornerMiddle);
                    }
                }
            }

            // Do the same but this time if we have inner/outer corners
            if (wallPrefabProfile.hasLCornerPrefabs)
            {
        ...
        private GameObject spawnPillar(Vector3 parentPosition, WallPiece parentWallPiece, ModularWallRelativeTransform pillarRT, ModularWallPillarRole pillarRole)
        {
            ...
            PluginPrefab pluginPrefab   = wallPrefabProfile.pickPillarPrefab(pillarRole);
            if (!pluginPrefab) return null;
        ...
        {
            if (wallPrefabProfile.hasLCornerPrefabs)
                return spawnPillar(firstCornerPiece.gameObject.transform.position, firstCornerPiece, pillarRT, ModularWallPillarRole.OuterCornerMiddle);
            else
            {
                ...
                OBB firstPieceOBB           = wallPrefabProfile.calcWallSegmentOBB(firstCornerPiece.gameObject);
                OBB secondPieceOBB          = wallPrefabProfile.calcWallSegmentOBB(secondCornerPiece.gameObject);
                ...
                PluginPrefab pluginPrefab   = wallPrefabProfile.pickPillarPrefab(ModularWallPillarRole.OuterCornerMiddle);
                if (!pluginPrefab) return null;
        ...
        {
            if (wallPrefabProfile.hasLCornerPrefabs)
                return spawnPillar(firstCornerPiece.gameObject.transform.position, firstCornerPiece, pillarRT, ModularWallPillarRole.InnerCornerMiddle);
            else
            {
                ...
                Vector3 objectPosition      = parentPosition + parentRotation * pillarRT.position;
                Quaternion objectRotation   = parentRotation * pillarRT.rotation;
                PluginPrefab pluginPrefab   = wallPrefabProfile.pickPillarPrefab(ModularWallPillarRole.InnerCornerMiddle);
                if (!pluginPrefab) return null;
        ...
        private void fixPillarPositionAndRotation(GameObject pillar, WallPiece cornerPiece, WallPiece otherCornerPiece, ModularWallRelativeTransform pillarRT)
        {
            ...
            {
                OBB otherPieceOBB = wallPrefabProfile.calcWallSegmentOBB(otherCornerPiece.gameObject);
                if (otherPieceOBB.isValid && pillarOBB.intersectsOBB(otherPieceOBB))
                {
                    ...
                    OBB wallPieceOBB            = wallPrefabProfile.calcWallSegmentOBB(cornerPiece.gameObject); 
