...
namespace GSPAWN
{
    public class UIShortcutItem : ListViewItem<Shortcut>
    {
        ...
        {
            RegisterCallback<ClickEvent>(p =>
            {
                if (data.canChangeFromUI && FixedShortcuts.ui_BeginHotkeyAssignmentOnClick(p)) beginHotkeyAssign();
            });
            ...
            _hotkeyAssignField.RegisterCallback<KeyDownEvent>(p =>
            {
                ...
                else
                {
                    if (p.keyCode != KeyCode.LeftCommand && p.keyCode != KeyCode.RightCommand &&
                        ...
                        p.keyCode != KeyCode.LeftAlt && p.keyCode != KeyCode.RightAlt)
                    {
                        endHotkeyAssign(p);
