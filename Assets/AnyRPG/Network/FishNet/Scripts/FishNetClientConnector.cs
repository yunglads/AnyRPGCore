...
namespace AnyRPG {
    public class FishNetClientConnector : ConfiguredNetworkBehaviour {
        ...
        public UnitController SpawnCharacterUnit(CharacterRequestData characterRequestData, Transform parentTransform, Vector3 position, Vector3 forward, Scene scene) {
            ...
            if (unitProfile == null) {
                Debug.LogWarning($"Could not find UnitProfile with name {characterRequestData.characterConfigurationRequest.unitProfile.ResourceName}");
                return null;
            }
            ...
            if ((characterRequestData.characterConfigurationRequest.unitControllerMode == UnitControllerMode.Mount
                ...
                if (clientId < 0) {
                    Debug.LogWarning($"Could not find clientId for accountId {characterRequestData.accountId}");
                    return null;
                }
        ...
        */
        ...
        private void SpawnPrefab(NetworkObject networkObject, NetworkConnection networkConnection, Scene scene) {
            //Debug.Log($"FishNetClientConnector.SpawnPrefab(object: {networkObject.gameObject.name}, client: {(networkConnection == null ? "null" : networkConnection.ClientId)}, sceneName: {scene.name}(sceneHandle: {scene.handle})) frame: {Time.frameCount} tick: {base.TimeManager.Tick}");
